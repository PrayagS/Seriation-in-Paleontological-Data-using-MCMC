...
def mcmc_randomize(model, r):
    ...
    cdef list all_sites = [i for i in range(model.N)]
    cdef list hard_sites = []  # contains Nh sites randomly chosen from all sites
    ...
    cdef list temp_arr = [all_sites[i] for i in range(model.N - model.Nh)]
    r.shuffle(temp_arr)
...
def check_valid_permutation(arr, model):
    cdef list hash_arr = [0 for i in range(max(arr) + 1)]
...
def check_consistency(model):
    ...
    cdef int t0 = model.tr0a
    cdef int f0 = model.fa0a
    cdef int t1 = model.tr1a
    cdef int f1 = model.fa1a
    cdef double loglike = model.loglike
    model.tr0, model.tr1, model.fa0, model.fa1, model.tr0a, model.tr1a, model.fa0a, model.fa1a = compute_trfa_count(
        model)
    model.loglike = compute_loglike(model)
    cdef double delta = loglike - model.loglike
    if (delta < np.double(0)):
        delta = -delta
